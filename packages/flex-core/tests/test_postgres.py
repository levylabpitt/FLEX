...
@pytest.fixture
def store():
    from flex.db.postgres import PostgresStore
...
def test_lifecycle(store):
    ...
    store.record_experiment_start(
        ExperimentRecord(id="e1", user="u", name="demo", start_time=t0, station="cryo1", config={"a": 1})
    )
    ...
    assert exp.user == "u" and exp.end_time == t1 and exp.station == "cryo1"
