...
Port of FLEX v1 ``flex.inst.levylab.TC_CF`` and ``TC_MNK`` (author: Pubudu
Wijesinghe <pubudu.wijesinghe@levylab.org>). Both are passive cooling
systems: reading temperature and heater power is supported, but
``set_temperature`` is deliberately disabled (manual gas handling is
required to change base temperature).
"""
