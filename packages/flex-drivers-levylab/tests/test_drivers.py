...
from flex_drivers_levylab.aerotech import Aerotech
from flex_drivers_levylab.cryostation import Cryostation
from flex_drivers_levylab.krohn_hite import KrohnHite7008
from flex_drivers_levylab.lockin import Lockin
from flex_drivers_levylab.opticool import Opticool
from flex_drivers_levylab.oxford import Oxford1820, OxfordVRM
from flex_drivers_levylab.ppms import PPMS
from flex_drivers_levylab.tc import TC_CF, TC_MNK
from flex_drivers_levylab.transport_server import TransportServer
...
def test_transport_server_expt_param_validation(server):
    with TransportServer(address=server.address) as ts:
        ...
        with pytest.raises(TypeError, match="must contain only str, int, or float"):
            ts.set_expt_param("bad", [1, "a"])
