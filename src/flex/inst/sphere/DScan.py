...
class DScan:
    library_name = Path(os.environ['ProgramFiles']).joinpath("Sphere Ultrafast Photonics S.A", "dscan-library", "bin", "dscan-library.dll")

    _lib = ctypes.cdll.LoadLibrary(os.path.join(os.path.dirname(os.path.realpath(__file__)), library_name))

    def __init__(self, serial_number: str = 'DM240913'):
