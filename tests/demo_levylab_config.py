"""FLEX demo: the LevyLab example config — real lab config, both driver families.

Run it from the repo root:

    uv run python tests/demo_levylab_config.py
...
Config note -- what's overridden and why:
  ...
  this dev machine can't reach. Everything else (data.writer = "tdms",
...
demo_default_config.py -- SQLite metadata + local files under
flex.config.default_data_root() -- except measurement data is TDMS
(.tdms), the real levylab.toml's [data] writer choice, not HDF5.

Nothing here touches your real active config
...
# -----------------------------------------------------------------------------
section("Loading the real 'levylab' example config")
# -----------------------------------------------------------------------------
manifest = Path(__file__).resolve().parent.parent / "examples" / "levylab.toml"
print(f"Config file:  {manifest}")
...
# -----------------------------------------------------------------------------
from flex_drivers.colby.pdl import ColbyPDL  # noqa: E402
